...
{
    global $dsql;
    $configfile = GEMELDATA.'/config.cache.inc.php';
...
else if($action=='setupstart')
{
    ...
    {
        include(GEMELDATA.'/module/'.$filename);
        exit();
    }
    //系统自动安装
    else
...
else if($action=='uninstallok')
{
    ...
    {
        include(GEMELDATA."/module/{$hash}-uninstall.php");
        @unlink(GEMELDATA."/module/{$hash}-uninstall.php");
        exit();
    }
    else
    {
...
else if($action=='view')
{
    ...
    if(file_exists(GEMELDATA."/module/{$hash}-readme.php")) 
    {
