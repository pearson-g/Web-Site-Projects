...
//生成必须同步服务器的列表
if(file_exists(GEMELDATA.'/config.file.inc.php'))
{
    require_once(GEMELDATA.'/config.file.inc.php');
}
...
{
    //将其转换为配置写入文件
    $configfile = GEMELDATA.'/config.file.inc.php';
...
else if($dopost == 'make')
{
    if($step == 1)
    {
        ...
        $cacheMakeFile = GEMELDATA.'/cache/filelist.inc.php';
        ...
        if(count($Iterm) > 0)
        {
            ...
            foreach($Iterm as $key => $val)
            {
                ...
                if(is_array($dirlist = getDirs(GEMELROOT.$val)))
                {
    ...
    {
        if(file_exists(GEMELDATA.'/cache/filelist.inc.php'))
        {
            require_once(GEMELDATA.'/cache/filelist.inc.php');
        }
        if(is_array($dirlist))
        {
            if($sta > 0 && $sta < $totalnum)
            {
                ...
                if($ftp->connect($config))
                {
                    ...
                    {
                        //如果是文件目录
                        $remotedir = str_replace(GEMELROOT, '', $dirname).'/';
                        $localdir = '..'.$remotedir.'/';
                        ...
                        $remotefile = str_replace(GEMELROOT, '', $dirname);
                        ...
                        {
                            list($filename,$fileconfig) = explode('|', $val); 
                            if(preg_replace('/[^\/]*\.(\w){0,}/', '', str_replace(GEMELROOT, '', $filename)) == $remotedir)
                            {
                ...
                @unlink(GEMELDATA.'/cache/filelist.inc.php');
                echo '全部同步完毕!';exit;
            }
        } else {
...
include GEMELINClude('templets/makeremote_all.htm');