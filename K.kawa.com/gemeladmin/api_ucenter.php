...
if(file_exists(GEMELROOT.'/uc_client/client.php'))
{
	if(!defined('UC_API')) define('UC_API', '');
	include_once GEMELROOT.'/uc_client/client.php';
}
else
{
...
class api_ucenter
{
	...
	function __construct($ac = '')
	{
		...
		$this->dtp = new GemelTemplate();
		$this->config = GEMELINC.'/common.inc.php';
		$this->$action();
	}
	...
	function uc_show()
	{
		...
		if(!defined('UC_APPID'))
		{
			$this->dtp->LoadTemplate(GEMELADMIN.'/templets/api_ucenter_install.htm');
		}
		else
		{
			$uc_api_open = false;			
			...
			$this->dtp->LoadTemplate(GEMELADMIN.'/templets/api_ucenter_edit.htm');
		}
		$this->dtp->Display();
		exit();
	}
