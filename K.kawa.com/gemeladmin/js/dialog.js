...
function editTitle(aid){
    ...
    var myajax = new GemelAjax(show,false,false,"","","");
    myajax.SendGet2("catalog_edit.php?dopost=time&id="+aid);
    GemelXHTTP = null;
}

function $Gemel(id){ return document.getElementById(id)}
function AlertMsg(title,id){
	...
	$Gemel("msgdiv").appendChild(thObj);
	$Gemel("msgdiv").appendChild(bodyObj);
	editTitle(id);
}
function CloseMsg(){
	//移除对象
	document.body.removeChild($Gemel("maskdiv")); 
	$Gemel("msgdiv").removeChild($Gemel("msgth")); 
	$Gemel("msgdiv").removeChild($Gemel("msgbody")); 
	document.body.removeChild($Gemel("msgdiv")); 
}
//拖动窗口
var ie = document.all;   
