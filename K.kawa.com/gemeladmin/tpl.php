...
if($action == 'edit' || $action == 'newfile')
{
    ...
    //获取标签帮助信息
    $helps = $dtags = array();
    $tagHelpDir = GEMELINC.'/taglib/help/';
    $dir = dir($tagHelpDir);
    ...
    foreach($dtags as $tag)
    {
        ...
        fclose($fp);
        $helps[$tag] = explode('>>gemel>>', $helpContent);
    }

    include GEMELADMIN.'/templets/tpl_edit.htm';
    exit();
}
/*---------------------------
function save_tpl() { }
...
?".'>';
    ...
    else
    {
        ...
        $fp = fopen(GEMELINC.'/taglib/'.$filename,'r');
        $democode = fread($fp, filesize(GEMELINC.'/taglib/'.$filename));
        fclose($fp);
        $title = "修改标签";
    }
    include GEMELADMIN.'/templets/tpl_edit_tag.htm';
    exit();
}
/*---------------------------
...
else if($action=='savetagfile')
{
    ...
    $content = stripslashes($content);
    $truefile = GEMELINC.'/taglib/'.$filename;
    $fp = fopen($truefile, 'w');
    fwrite($fp, $content);
    fclose($fp);
    $msg = "
    <form name='form1' action='tag_test_action.php' target='blank' method='post'>
      <input type='hidden' name='dopost' value='make' />
        ...
        <textarea name='partcode' cols='150' rows='6' style='width:90%;'>{gemel:{$tagname} }{/gemel:{$tagname}}</textarea><br />
