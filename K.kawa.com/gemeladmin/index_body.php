...
require(GEMELINC.'/image.func.php');
require(GEMELINC.'/gemeltag.class.php');
...
if(empty($dopost))
{
    ...
    $myMoveFile = GEMELDATA.'/admin/move-'.$cuserLogin->getUserID().'.txt';
    if(file_exists($myMoveFile))
    {
        ...
        include GEMELINClude('templets/index_body_move.htm');
    }else{  
        include GEMELINClude('templets/index_body.htm');
    }
    exit();
}
/*-----------------------
...
else if($dopost=='addnew')
{
    ...
    $myIcoFileTrue = GEMELDATA.'/admin/quickmenu-'.$cuserLogin->getUserID().'.txt';
...
{   
    ...
    $myIcoFileTrue = GEMELDATA.'/admin/move-'.$cuserLogin->getUserID().'.txt';
    $fp = fopen($myIcoFileTrue,'w');
    fwrite($fp,$movedata);
    fclose($fp);
}
...
} else if($dopost=='setskin')
{
	...
	$skinconfig = GEMELDATA.'/admin/skin.txt';
