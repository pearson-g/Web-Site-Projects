...
require_once(GEMELINC."/customfields.func.php");
...
if($dopost!='save')
{
    require_once(GEMELADMIN."/inc/inc_catalog_options.php");
    ...
    $tags = GetTags($aid);
    include GEMELINClude("templets/album_edit.htm");
    exit();
}
/*--------------------------------
function __save(){  }
...
else if($dopost=='save')
{
    ...
    $keywords = trim(cn_substrR($keywords,250));
    $filename = trim(cn_substrR($filename,200));
    ...
    $imgurls = "{gemel:pagestyle maxwidth='$maxwidth' pagepicnum='$pagepicnum' ddmaxwidth='$ddmaxwidth' row='$row' col='$col' value='$pagestyle'/}\r\n";
    $hasone = false;
    ...
    for($i=1; $i<=120; $i++)
    {
        ...
        if( isset(${'imgfile'.$i}) && is_uploaded_file(${'imgfile'.$i}) )
        {
            ...
            if(!is_array($imginfos))
            {
                ...
                $imgurls .= "{gemel:img ddimg='$ddurl' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/gemel:img}\r\n";
                continue;
            }
            ...
            $imgurls .= "{gemel:img ddimg='$litpicname' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/gemel:img}\r\n";
        }
        //没上传图片(只修改msg信息)
        else
        {
    ...
    if($formzip==1)
    {
        include_once(GEMELINC."/zip.class.php");
        include_once(GEMELADMIN."/file_class.php");
        $zipfile = $cfg_basedir.str_replace($cfg_mainsite,'',$zipfile);
        $tmpzipdir = GEMELDATA.'/ziptmp/'.cn_substr(md5(ExecTime()),16);
        $ntime = time();
        if(file_exists($zipfile))
        {
            ...
            foreach($imgs as $imgold)
            {
                ...
                if(is_file($imgfile))
                {
                    ...
                    $info = '';
                    $imginfos = GetImageSize($imgfile,$info);
                    $imgurls .= "{gemel:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/gemel:img}\r\n";
    ...
    if(is_array($_SESSION['bigfile_info']))
    {
        foreach($_SESSION['bigfile_info'] as $k=>$v)
        {
            ...
            $imgurls .= "{gemel:img ddimg='$litpicname' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $v {/gemel:img}\r\n";
        }
    }
    ...
    $inadd_f = '';
    $inadd_v = '';
    if(!empty($gemel_addonfields))
    {
        $addonfields = explode(';', $gemel_addonfields);
        $inadd_f = '';
        $inadd_v = '';
