...
require_once(GEMELADMIN.'/inc/inc_list_functions.php');
...
if($upate_weight == 'Yes')
{
	...
	$local_url = 'content_list_pro.php?dopost=listArchives&keyword='.$keyword.'&cid='.$cid.'&flag='.$flag.'&orderby='.$orderby.'&arcrank='.$arcrank.'&channelid='.$channelid.$page_url;
...
else
{
  ...
  $optCache = GEMELDATA."/tplcache/inc_option_$optHash.inc";
  ...
  if($arcrank != '')
  {
	  ...
	  $CheckUserSend = "<input type='button' class='coolbg np' onClick=\"location='catalog_do.php?cid=".$cid."&dopost=listArchives&gurl=content_list_pro.php';\" value='所有文档' />";
  }
  else
  {
	  $CheckUserSend = "<input type='button' class='coolbg np' onClick=\"location='catalog_do.php?cid=".$cid."&dopost=listArchives&arcrank=-1&gurl=content_list_pro.php';\" value='稿件审核' />";
  }
  ...
  //模板
  if(empty($s_tmplets)) $s_tmplets = 'templets/content_list_pro.htm';
  $dlist->SetTemplate(GEMELADMIN.'/'.$s_tmplets);
