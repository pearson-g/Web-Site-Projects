...
if(file_exists(GEMELDATA.'/admin/skin.txt'))
{
	$skin = file_get_contents(GEMELDATA.'/admin/skin.txt');
	$cfg_admin_skin = !in_array($skin, array(1,2,3,4))? 1 : $skin;
}

//获得当前脚本名称，如果你的系统被禁用了$_SERVER变量，请自行更改这个选项
$gemelNowurl = $s_scriptName = '';
...
{
    header("location:login.php?gotopage=".urlencode($gemelNowurl));
    exit();
}
...
if($cfg_gemel_log=='Y')
{
    ...
    $s_query = isset($gemelNowurls[1]) ? $gemelNowurls[1] : '';
...
//管理缓存、管理员频道缓存
$cache1 = GEMELDATA.'/cache/inc_catalog_base.inc';
if(!file_exists($cache1)) UpDateCatCache();
$cacheFile = GEMELDATA.'/cache/admincat_'.$cuserLogin->userID.'.inc';
if(file_exists($cacheFile)) require_once($cacheFile);

//更新服务器
require_once (GEMELDATA.'/admin/config_update.php');
...
function UpDateCatCache()
{
    ...
    $cache2 = GEMELDATA.'/cache/channelsonlist.inc';
    $cache3 = GEMELDATA.'/cache/channeltoplist.inc';
...
{
    global $dsql;
    $cachefile = GEMELDATA.'/cache/member_model.inc';
 ...
 * @return    string
 */
function GEMELINClude($filename, $isabs=FALSE)
{
    return $isabs ? $filename : GEMELADMIN.'/'.$filename;
}
