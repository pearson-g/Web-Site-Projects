...
@set_time_limit(0);
require(GEMELINC.'/inc/inc_fun_funAdmin.php');
...
else if($dopost=='getlist')
{
    ...
    include GEMELINClude('templets/update_guide_getlist.htm');
    exit();
}
/**
下载文件（保存需下载内容列表）
...
{
    $cacheFiles = GEMELDATA.'/cache/updatetmp.inc';
    ...
    else
    {
        ...
        foreach($files as $filename)
        {
            ...
            {
                $tfilename = preg_replace("#^gemeladmin\/#", $adminDir.'/', $filename);
            }
    ...
    include GEMELINClude('templets/update_guide_getfiles.htm');
    exit();
}
/**
下载文件，具体操作步骤
...
else if($dopost=='down')
{
    ...
    else
    {
        ...
        $dhd->Close();
        $truefile = GEMELDATA.'/'.$tmpdir.'/sql.txt';
        $fp = fopen($truefile, 'w');
        fwrite($fp, $ct);
        fclose($fp);
...
else if($dopost=='apply')
{
    ...
    else
    {
        ...
        if(isset($files) && is_array($files))
        {
            foreach($files as $f)
            {
                if(preg_match('#^gemeladmin#', $f)) 
                {
                    $tf = preg_replace('#^gemeladmin#', $adminDir, $f);
                }
                else {
                    $tf = $f;
                }
