...
else if($dopost=="export")
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
    $channelconfig = '';
    $row['maintable'] = preg_replace('#gemel_#', '#@__', $row['maintable']);
    $row['addtable'] = preg_replace('#gemel_#', '#@__', $row['addtable']);
    foreach($row as $k=>$v)
    {
        if($k=='fieldset') $v = "\r\n$v\r\n";
...
{
    require_once(GEMELADMIN."/inc/inc_admin_channel.php");
...
$row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
require_once(GEMELADMIN."/templets/mychannel_edit.htm");