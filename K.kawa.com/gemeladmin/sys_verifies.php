...
@set_time_limit(0);
require(GEMELINC.'/gemelhttpdown.class.php');

$action = isset($action) ? trim($action) : '';

//当前软件版本锁定文件
$verLockFile = GEMELDATA.'/admin/ver.txt';
//当前软件指纹码锁定文件
$verifiesLockFile = GEMELDATA.'/admin/verifies.txt';
...
if($action == '')
{
    include(GEMELADMIN.'/templets/sys_verifies.htm');
    exit();
}
/*----------------
校验文件
...
else if($action == 'verify')
{
    ...
    {
        $turefile = str_replace('../gemeladmin', '.', $row['filename']);
        //跳过不存在的文件
    ...
    {
        include(GEMELADMIN.'/templets/sys_verifies_verify.htm');
    }
    exit();
}
/*--------------------
...
else if ($action == 'manage')
{
    ...
    include(GEMELADMIN.'/templets/sys_verifies_manage.htm');
    exit();
}
/*-----------------------
下载文件
...
else if ($action == 'getfiles')
{
    ...
    $cacheFiles = GEMELDATA.'/modifytmp.inc';
    ...
    foreach($refiles as $filename)
    {
        ...
        if(preg_match("#^gemeladmin/#i", $filename)) 
        {
            $curdir = GetDirName( preg_replace("#^gemeladmin/#i", $adminDir.'/', $filename) );
        } else {
    ...
    include(GEMELADMIN.'/templets/sys_verifies_getfiles.htm');
...
else if($action=='down')
{
    ...
    $dhd = new GemelHttpDown();
    $dhd->OpenUrl($downfile);
    $dhd->SaveToBin(GEMELDATA.'/'.$tmpdir.'/'.$files[$curfile]);
    $dhd->Close();
...
else if($action=='modify')
{
    ...
    else
    {
        foreach($modifys as $fname)
        {
            if($method=='local')
            {
                $tureFilename = str_replace('../gemeladmin','./',$fname);
...
else if ($action == 'apply')
{
    ...
    if(isset($files) && is_array($files))
    {
        foreach($files as $f)
        {
            if(preg_match("#^gemeladmin#", $f)) $tf = preg_replace("#^gemeladmin#", $adminDir, $f);
            else $tf = $f;
