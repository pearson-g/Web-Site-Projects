 ...
 */
require_once(GEMELINC.'/gemelhttpdown.class.php');
require_once(GEMELINC.'/image.func.php');
require_once(GEMELINC.'/archives.func.php');
...
function GetCurContentAlbum($body, $rfurl, &$firstdd)
{
    ...
    require_once(GEMELINC.'/gemelcollection.func.php');
    ...
    foreach($img_array as $key=>$value)
    {
        ...
        if($rs)
        {
            ...
            $rsimg .= "{gemel:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/gemel:img}\r\n";
        }
    }
    return $rsimg;
}
...
{
    global $envs, $typeid;
    require_once(GEMELINC.'/arc.archives.class.php');
...
function PrintAutoFieldsAdd(&$fieldset, $loadtype='all')
{
    ...
    if(is_array($dtp->CTags))
    {
        foreach($dtp->CTags as $tid=>$ctag)
        {
            ...
            {
                $gemel_addonfields .= ( $gemel_addonfields=="" ? $ctag->GetName().",".$ctag->GetAtt('type') : ";".$ctag->GetName().",".$ctag->GetAtt('type') );
                echo  GetFormItemA($ctag);
            }
        }
    }
    echo "<input type='hidden' name='gemel_addonfields' value=\"".$gemel_addonfields."\">\r\n";
}
...
function AnalyseHtmlBody($body,&$description,&$litpic,&$keywords,$dtype='')
{
    ...
    if($dellink==1)
    {
        ...
        // 读取允许的超链接设置
        if(file_exists(GEMELDATA."/admin/allowurl.txt"))
        {
            $allow_urls = array_merge($allow_urls, file(GEMELDATA."/admin/allowurl.txt"));
        }
    ...
    if($autokey==1 && $keywords=='')
    {
        $subject = $title;
        $message = $body;
        include_once(GEMELINC.'/splitword.class.php');
        $keywords = '';
