...
else if($action=='' || $action=='show')
{
	...
	$dlist->SetTemplate($cfg_basedir.$cfg_templets_dir.'/module/bookfeedback_templet.htm');
...
else if($action=='quote')
{
	...
	$dtp->LoadTemplate($cfg_basedir.$cfg_templets_dir.'/module/bookfeedback_quote.htm');
	$dtp->Display();
	exit();
}
//发表评论
...
else if($action=='send')
{
	...
	if($isconfirm!='yes' && $cfg_feedback_ck=='Y')
	{
		...
		require_once(GEMELINC.'/gemeltemplate.class.php');
		$dtp = new GemelTemplate();
		$dtp->LoadTemplate($cfg_basedir.$cfg_templets_dir.'/module/bookfeedback_confirm.htm');
		$dtp->Display();
		exit();
	}
	//检查验证码
	...
	if($cfg_ml->M_ID > 0)
	{
		...
		if(defined('UC_APPID'))
		{
			include_once GEMELROOT.'/api/uc.func.php';
