 ...
 * 评论
 *
 * @version        $Id: feedback.php 1 15:38 2010年7月8日Z 
...
require_once(GEMELINC."/filter.inc.php");
if(!isset($action))
{
    $action = '';
}
...
else if($action=='' || $action=='show')
{
    ...
    $dlist->SetTemplate(GEMELTEMPLATE.'/module/feedback_templet.htm');
...
else if($action=='quote')
{
	...
	$tplfile = $type == ''? GEMELTEMPLATE.'/module/feedback_quote.htm' : GEMELTEMPLATE.'/module/feedback_quote_ajax.htm';
...
else if($action=='send')
{
    ...
    if($isconfirm!='yes' && $cfg_feedback_ck=='Y')
    {
        ...
        require_once(GEMELINC.'/gemeltemplate.class.php');
        $dtp = new GemelTemplate();
        $dtp->LoadTemplate(GEMELTEMPLATE.'/module/feedback_confirm.htm');
        $dtp->Display();
        exit();
    }
    //检查验证码
