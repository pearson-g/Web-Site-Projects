...
include_once(GEMELINC.'/memberlogin.class.php');
...
{
    require_once(GEMELINC.'/charset.func.php');
    ...
    else if($username!='' && $pwd!='')
    {
        ...
        if($rs==1)
        {
            $dsql->ExecuteNoneQuery("Update `#@__member_list` set logintime='".time()."',loginip='".GetIP()."' where mid='{$cfg_ml->M_ID}'; ");
        }
        $cfg_ml = new MemberLogin();
    }
  ...
  //给文章评分
    ...
    {
        #api{{
        if(defined('UC_API') && @include_once GEMELROOT.'/api/uc.func.php')
        {
            //同步积分
        ...
        #/aip}}
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_list` set scores=scores+{$cfg_sendfb_scores} WHERE mid='{$cfg_ml->M_ID}' ");
    ...
    else
    {
        ...
        if($cfg_ml->M_ID==""){
          ...
          $row = $dsql->GetOne("SELECT face,sex FROM `#@__member_list` WHERE mid={$cfg_ml->M_ID} ");
            if(empty($row['face']))
            {
              if($row['sex']=="女") $mface=$cfg_cmspath."/user/templets/images/dfgirl.png";
              else $mface=$cfg_cmspath."/user/templets/images/dfboy.png";
            }
        }
?>
  ...
  <ul>
    ...
    <div class="comment_act"><span class="fl"><?php echo GetDateMk($dtime); ?>发表</span></div>
      ...
      <div class="newcomment_act"><span class="fr"><span id='goodfb<?php echo $id; ?>'> <a href='#goodfb<?php echo $id; ?>' onclick="postBadGood('goodfb',<?php echo $id; ?>);">支持</a>[0] </span> <span id='badfb<?php echo $id; ?>'> <a href='#badfb<?php echo $id; ?>' onclick="postBadGood('badfb',<?php echo $id; ?>);">反对</a>[0] </span> <span class='quote'>
        <!--<a href='/module/feedback.php?aid=<?php echo $id; ?>&fid=<?php echo $id; ?>&action=quote'>[引用]</a>-->
...
function GetList($page=1)
{
    ...
    $querystring = "SELECT fb.*,mb.userid,mb.face as mface,mb.spacesta,mb.scores,mb.sex FROM `#@__feedback` fb
                 LEFT JOIN `#@__member_list` mb on mb.mid = fb.mid WHERE fb.aid='$aid' AND fb.ischeck='1' ORDER BY fb.id DESC";
    ...
    while($fields = $dsql->GetArray('fb'))
    {
        ...
        {
            if($fields['sex']=="女") $fields['mface']=$cfg_cmspath."/user/templets/images/dfgirl.png";
            else $fields['mface']=$cfg_cmspath."/user/templets/images/dfboy.png";
        }
