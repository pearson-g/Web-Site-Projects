 ...
 * 模板类
 *
 * @version        $Id: gemeltag.class.php 1 10:33 2010年7月6日
 */

/**
 * class GemelTag 标记的数据结构描述
 * function c___GemelTag();
 *
 * @package          GemelTag
 */ 
...
/**
 * GemelTagParse 模板类
 * @package          GemelTagParse
 */
class GemelTagParse
{
    var $NameSpace = 'gemel';   //标记的名字空间
...
// ------------------------------------------------------------------------
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = GEMELROOT.$cfg_tplcache_dir;
        ...
        if( isset($z) && is_array($z) )
        {
            foreach($z as $k=>$v)
            {
                $this->Count++;
                $ctag = new GemelTag();
                $ctag->CAttribute = new GemelAttribute();
    ...
    function LoadSource($str)
    {
        ...
        */
        //优化模板字符串存取读取方式
        $this->taghashfile = $filename = GEMELDATA.'/tplcache/'.md5($str).'.inc';
        if( !is_file($filename) )
        {
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        {
            $GemelMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$GemelMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $GemelMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
    ...
    function SaveTo($filename)
    {
		//echo $filename;
		//exit;
        $fp = @fopen($filename,"w") or die("GemelTag Engine Create File False");
        ...
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                //found '/}' and found '{/gemel:'
                else
                {
                    //if '/}' more near '{gemel:'、'{/gemel:' , end tag is '/}', else is '{/gemel:'
                    if($e1 < $e2 &&  $e1 < $e3 )
                    {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
    ...
    {
        $GemelFieldValue = $fieldvalue;
        $functionname = str_replace("{\"","[\"",$functionname);
        $functionname = str_replace("\"}","\"]",$functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i",'$GemelFieldValue',$functionname);
        $functionname = "\$GemelFieldValue = ".$functionname;
        @eval($functionname.";"); //or die("<xmp>$functionname</xmp>");
        if(empty($GemelFieldValue))
        {
            return '';
        }
        else
        {
            return $GemelFieldValue;
        }
    }
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(GEMELROOT."/templets/".$filename) )
        {
            $okfile = GEMELROOT."/templets/".$filename;
        }
        else if(file_exists(GEMELROOT.'/templets/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = GEMELROOT.'/templets/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
			$okfile = GEMELROOT.'/templets/'.$cfg_df_style.'/'.$filename;
...
/**********************************************
//class GemelAttribute 模板标记属性集合
**********************************************/
//属性的数据描述
class GemelAttribute
{
    var $Count = -1;
    var $Items = ""; //属性元素的集合
    //获得某个属性
...
class GemelAttributeParse
{
    ...
    {
        $this->cAttributes = new GemelAttribute();
        $strLen = 0;
