			...
			if (isset($_REQUEST['tid']))
			{
				foreach($_GET as $key => $value) {
                    ...
                    if($key=="pro_certificate" || $key=="pro_material" || $key=="pro_temperature" || $key=="pro_voltage"){//你自定义字段的字段名(英文的那个name)
                        ...
                        $count=0;
                        $nowsql ='';
                        //$var=explode(",",wwwcms_filter(iconv("UTF-8", "GB2312//IGNORE", $value)));//iconv是转码的意思,如果你url传进来的参数是中文,就需要转码
                        $var=explode(",",$value);
                        // print_r($var);
                        if(!empty($var)){
        ...
        if($addtable!="")
        {
			...
			if (isset($_REQUEST['tid']))
			{
    			foreach($_GET as $key => $value) {
                    ...
                    //echo 22;exit();pro_material,pro_season,pro_style,pro_types
                    if($key=="pro_certificate" || $key=="pro_material" || $key=="pro_temperature" || $key=="pro_voltage"){//你自定义字段的字段名(英文的那个name)
                        //echo 11;exit();
                        $count=0;
                        $nowsql ='';
