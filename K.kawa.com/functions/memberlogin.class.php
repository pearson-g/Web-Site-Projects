...
class MemberLogin
{
    ...
    function __construct($kptime = -1, $cache=FALSE)
    {
        ...
        $formcache = FALSE;
        $this->M_ID = $this->GetNum(GetCookie("GemelUserID"));
        $this->M_LoginTime = GetCookie("GemelLoginTime");
        $this->fields = array();
        $this->isAdmin = FALSE;
        if(empty($this->M_ID))
        {
            ...
            if(is_array($this->fields)){
                #api{{
                if(defined('UC_API') && @include_once GEMELROOT.'/uc_client/client.php')
                {
                ...
                if(time() - $this->M_LoginTime > 3600)
                {
                    ...
                    PutCookie("GemelLoginTime",time(),$this->M_KeepTime);
                }
    ...
    function PutLoginInfo($uid, $logintime=0)
    {
        ...
        {
            PutCookie('GemelUserID',$uid,$this->M_KeepTime);
            PutCookie('GemelLoginTime',$this->M_LoginTime,$this->M_KeepTime);
        }
        else
        {
            PutCookie('GemelUserID',$uid);
            PutCookie('GemelLoginTime',$this->M_LoginTime);
        }
    }
