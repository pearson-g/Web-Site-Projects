...
$dsql = $dsqli = $db = new GemelSqli(FALSE);
/**
 * MySQLi数据库类
 */
...
class GemelSqli
{
    ...
    function GemelSql($pconnect=FALSE,$nconnect=TRUE)
    {
    ...
    //用指定参数初始数据库信息
    function SetSource($host,$username,$pwd,$dbname,$dbprefix="gemel_")
    {
        $this->dbHost = $host;
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = GEMELINC.'/../config/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();
