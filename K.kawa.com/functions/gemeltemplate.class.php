 ...
 * 模板引擎文件
 *
 * @version        $Id: gemeltemplate.class.php 3 15:44 2010年7月6日
 */
...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
    ...
    $plusfile = GEMELINC.'/tpllib/plus_'.$atts['tagname'].'.php';
...
/**
 * 模板解析器
 * function C__GemelTemplate
 *
 * @package          GemelTemplate
 */
class GemelTemplate
{
    var $tagMaxLen = 64;
    ...
    var $refObj = '';
    var $makeLoop = 0;
    var $tagStartWord =  '{gemel:';
    var $fullTagEndWord =  '{/gemel:';
    var $sTagEndWord = '/}';
    var $tagEndWord = '}';
    var $tpCfgs = array();
    ...
    function __construct($templatedir='',$refDir='')
    {
        ...
        if($templatedir=='')
        {
            $this->templateDir = GEMELROOT.'/templets';
        }
        else
        {
        ...
        $this->cacheDir = GEMELROOT.$GLOBALS['cfg_tplcache_dir'];
    }

    //构造函数,兼容PHP4
    function GemelTemplate($templatedir='',$refDir='')
    {
     ...
     * @return    void
     */
    function SetTagStyle($ts='{gemel:',$ftend='{/gemel:',$stend='/}',$tend='}')
    {
...
// ------------------------------------------------------------------------
    ...
    function CompilerInclude($filename, $isload=TRUE)
    {
        ...
        if($okfile=='') return 0;
        if( !$isload ) return 1;
        $itpl = new GemelTemplate($this->templateDir);
