...
function SpGetPinyin($str, $ishead=0, $isclose=1)
{
    ...
    if(count($pinyins) == 0)
    {
        $fp = fopen(GEMELINC.'/data/pinyin.dat', 'r');
        while(!feof($fp))
        {
            $line = trim(fgets($fp));
...
function SpGetEditor($fname,$fvalue,$nheight="350",$etype="Basic",$gtype="print",$isfullpage="false",$bbcode=false)
{
    ...
    {
        require_once(GEMELINC.'/FCKeditor/fckeditor.php');
    ...
    {
        require_once(GEMELINC.'/ckeditor/ckeditor.php');
        $CKEditor = new CKEditor();
        $CKEditor->basePath = $GLOBALS['cfg_cmspath'].'/functions/ckeditor/' ;
        $config = $events = array();
        $config['extraPlugins'] = 'gemelpage,multipic,addon';
		if($bbcode)
		{
			...
			// 获取表情信息
			require_once(GEMELDATA.'/smiley.data.php');
			$jsscript = array();
