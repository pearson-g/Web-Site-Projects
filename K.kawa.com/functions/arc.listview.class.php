...
class ListView
{
    ...
    function __construct($typeid, $uppage=1)
    {
        ...
        $this->IsError = false;
        $this->dtp = new GemelTagParse();
        $this->dtp->SetRefObj($this);
        $this->dtp->SetNameSpace("gemel", "{", "}");
        $this->dtp2 = new GemelTagParse();
    ...
    function CountRecord()
    {
		...
		if($addtable!="")
		{
			...
			{
                /*function checkbox($obj_id){
                    switch ($obj_id) {
                            case 2:
                              return $key=="pro_certificate" || $key=="pro_material" || $key=="pro_temperature" || $key=="pro_voltage"; 
                               //return 1;
                            break;
                            case 47:                        
                              return $key=="quick_type" || $key=="quick_orientation" || $key=="quick_width";
                            break;
                    }
                }*/
				...
				foreach($_GET as $key => $value) {
                    ...
                    switch ($_REQUEST['tid']) {
                            case 2: 
                            if($key=="pro_certificate" || $key=="pro_material" || $key=="pro_temperature" || $key=="pro_voltage"){//你自定义字段的字段名(英文的那个name)
                                //echo 11;exit();
                                $count=0;
                                $nowsql ='';
                            ...
                            break;
                            case 47:
                              if($key=="quick_type" || $key=="quick_orientation" || $key=="quick_width"){//你自定义字段的字段名(英文的那个name)
                                //echo 11;exit();
                                $count=0;
                                $nowsql ='';
                                //$var=explode(",",wwwcms_filter(iconv("UTF-8", "gb2312", $value)));//iconv是转码的意思,如果你url传进来的参数是中文,就需要转码
    ...
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0)
    {
        ...
        for($this->PageNo=$startpage; $this->PageNo < $endpage; $this->PageNo++)
        {
            ...
            {
                //分析远程文件路径
                $remotefile = str_replace(GEMELROOT, '',$makeFile);
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        if($addtable!="")
        {
			...
			if (isset($_REQUEST['tid']))
			{
    			foreach($_GET as $key => $value) {
                    ...
                    //echo 22;exit();pro_certificate,pro_material,pro_temperature,pro_voltage
                    switch ($_REQUEST['tid']) {
                            case 2: 
                            if($key=="pro_certificate" || $key=="pro_material" || $key=="pro_temperature" || $key=="pro_voltage"){//你自定义字段的字段名(英文的那个name)
                                //echo 11;exit();
        ...
        $t2 = ExecTime();
        $sqlToal=$this->dsql->SetQuery($query);
        
        /*$sqlToal=strstr($sqlToal,'LIMIT',true);*/
        $strlen = strlen($sqlToal);  //全部字符长度
        $tp = strpos($sqlToal,"LIMIT");  //limit之前的字符长度
        $sqlToal = substr($sqlToal,-$strlen,$tp);  //从头开始截取到指字符位置。
        
        // echo $this->TotalResult.'llllllllllll<br/>';
        $resultToal=mysql_query($sqlToal);
        $this->TotalResult=mysql_num_rows($resultToal);
        //echo $this->TotalResult.'-----<br/>';
        //echo $sqlToal.'<br/>';
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno",$site_lang)
    {
        ...
        if($list_len=='' || preg_match("/[^0-9]/", $list_len))
        {
         ...
         //echo $this->TotalResult.'zzzzzzzzz<br/>';
        $totalpage = ceil($this->TotalResult/$this->PageSize);
		if($site_lang == 'cn' || $site_lang =='')
   		{
			if($totalpage<=1 && $this->TotalResult>0)
		...
		elseif($site_lang == 'en')
		{
           // echo $this->TotalResult.'ssssssssssss<br/>';
			if($totalpage<=1 && $this->TotalResult>0)
			{
