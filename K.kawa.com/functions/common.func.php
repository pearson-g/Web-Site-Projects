 ...
 */
if(!defined('GEMELINC')) exit('Error');
...
function helper($helpers)
{
    ...
    if (file_exists(GEMELINC.'/helpers/'.$helpers.'.helper.php'))
    { 
        include_once(GEMELINC.'/helpers/'.$helpers.'.helper.php');
...
{    
    ...
    $imgurls= $row['imgurls'];
	preg_match_all("/{gemel:img (.*){\/gemel:img/isU",$imgurls,$wordcount); 
...
function RunApp($ct, $ac = '',$directory = '')
{
	...
	if(!empty($directory)) $path = GEMELCONTROL.'/'.$directory. '/' . $ct . '.php';
	else $path = GEMELCONTROL . '/' . $ct . '.php';
...
function UpdateStat()
{
    include_once(GEMELINC."/inc/inc_stat.php");
    return SpUpdateStat();
}
...
function ShowMsg($msg, $gourl, $onlymsg=0, $limittime=0)
{
    ...
    $htmlhead .= "<base target='_self'/>\r\n<style>div{line-height:160%;}</style></head>\r\n<body leftmargin='0' topmargin='0' style='background:#f1f1f1'>".(isset($GLOBALS['ucsynlogin']) ? $GLOBALS['ucsynlogin'] : '')."\r\n<center>\r\n<script>\r\n";
    $htmlfoot  = "</script>\r\n</center>\r\n</body>\r\n</html>\r\n";
...
// 自定义函数接口
if( file_exists(GEMELINC.'/extend.func.php') )
{
    require_once(GEMELINC.'/extend.func.php');
}
