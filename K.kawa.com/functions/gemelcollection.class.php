 ...
 * 采集类
 *
 * @version        $Id: gemelcollection.class.php 1 20:20 2010年7月7日
 */

require_once(GEMELINC."/gemelcollection.func.php"); //采集扩展函数
require_once(GEMELINC."/image.func.php");
require_once(GEMELINC."/gemelhtml.class.php");
@set_time_limit(0);
 ...
 * 采集类
 *
 * @package          GemelCollection
 */
class GemelCollection
{
    ...
    function __construct()
    {
        ...
        $this->cHttpDown = new GemelHttpDown();
        $this->cGemelHtml = new GemelHtml2();
    }

    function GemelCollection()
    {
        $this->__construct();
    }
    ...
    function GetSpPage($dourl, $noteid, $html, $step=0)
    {
        ...
        {
            $this->tmpUnitValue .= $this->GetHtmlArea('[内容]', $sarr['match'], $html);
            $this->cGemelHtml->GetLinkType = "link";
            $this->cGemelHtml->SetSource($linkareaHtml, $dourl, 'link');
            foreach($this->cGemelHtml->Links as $k=>$t)
            {
                $k = $this->cGemelHtml->FillUrl($k);
                if($k==$dourl)
                {
                    continue;
                }
        ...
        else if ($this->artNotes["sptype"] == 'diyrule')
        {
            ...
            for($i=$this->spNotes['srul']; $i <= $this->spNotes['erul']; $i++)
            {
                ...
                $tempurl = $this->cGemelHtml->FillUrl($tempurl);
    ...
    function GetPageFields($dourl, $needDown, $litpic='')
    {
        ...
        if(isset($inarr[1]))
        {
            ...
            $artitem .= "{gemel:field name='keywords'}".$keywords."{/gemel:field}\r\n";
        }
        else
        {
            $artitem .= "{gemel:field name='keywords'}{/gemel:field}\r\n";
        }
        ...
        if(isset($inarr[1]))
        {
            ...
            $artitem .= "{gemel:field name='description'}".$description."{/gemel:field}\r\n";
        }
        else
        {
            $artitem .= "{gemel:field name='description'}{/gemel:field}\r\n";
        }
        ...
        foreach($this->artNotes as $k=>$sarr)
        {
            ...
            else
            {
                $v = preg_replace("#(　)$#", '', $v);
                $v = preg_replace("#[\r\n\t ]{1,}$#", '', $v);
                $artitem .= "{gemel:field name='$k'}$v{/gemel:field}\r\n";
            }
        }//End Foreach
        ...
        {
            $artitem .= "{gemel:field name='litpic'}".$this->DownMedia($litpic,'img',TRUE)."{/gemel:field}\r\n";
        }
        else
        {
            $artitem .= "{gemel:field name='litpic'}".$this->breImage."{/gemel:field}\r\n";
        }
        return $artitem;
    }
    ...
    {
        $this->cGemelHtml->SetSource($html,$url,'media');

        //下载标记里的图片和flash
        foreach($this->cGemelHtml->Medias as $k=>$v)
        {
            $furl = $this->cGemelHtml->FillUrl($k);
        ...
        //下载超链接里的图片
        foreach($this->cGemelHtml->Links as $v=>$k)
        {
            ...
            $furl = $this->cGemelHtml->FillUrl($v);
    ...
    {
        $this->cGemelHtml->SetSource($html, $dourl, 'media');
        foreach($this->cGemelHtml->Medias as $k=>$v)
        {
            $k = trim($k);
            $okurl = $this->cGemelHtml->FillUrl($k);
            $html = str_replace($k, $okurl, $html);
        }
        return $html;
    }
        ...
        else
        {
            ...
            foreach($this->lists['url'] as $k=>$cururls)
            {
                ...
                if($moviePostion > $glstart)
                {
                    ...
                    $this->cGemelHtml->SetSource($html, $cururl, 'link');
                    $lk = 0;
                    foreach($this->cGemelHtml->Links as $k=>$v)
                    {
                        if($this->lists['nothas']!='')
                        {
    ...
    {
        $GemelMeValue = $fvalue;
        $phpcode = preg_replace("#'@me'|\"@me\"|@me#isU", '$GemelMeValue', $phpcode);
        if(preg_match("#@body#i", $phpcode))
        {
            $GemelBodyValue = $this->tmpHtml;
            $phpcode = preg_replace("#'@body'|\"@body\"|@body#isU", '$GemelBodyValue', $phpcode);
        }
        if(preg_match("#@litpic#i", $phpcode))
        {
            $GemelLitPicValue = $this->breImage;
            $phpcode = preg_replace("#'@litpic'|\"@litpic\"|@litpic#isU", '$GemelLitPicValue', $phpcode);
        }
        eval($phpcode.";");
        return $GemelMeValue;
    }
