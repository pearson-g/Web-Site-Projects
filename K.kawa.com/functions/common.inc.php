...
define('GEMELINC', str_replace("\\", '/', dirname(__FILE__) ) );
define('GEMELROOT', str_replace("\\", '/', substr(GEMELINC,0,-10) ) );
define('GEMELDATA', GEMELROOT.'/config');
define('GEMELMEMBER', GEMELROOT.'/user');
define('GEMELTEMPLATE', GEMELROOT.'/templets');
...
// ------------------------------------------------------------------------
define('GEMELMODEL', './model');
define('GEMELCONTROL', './control');
define('GEMELAPPTPL', './templates');
...
if($_FILES)
{
    require_once(GEMELINC.'/uploadsafe.inc.php');
}

//数据库配置文件
require_once(GEMELDATA.'/gemeldb.php');

//载入系统验证安全配置
if(file_exists(GEMELDATA.'/safe/inc_safe_config.php'))
{
...
//站点根目录
$cfg_basedir = preg_replace('#'.$cfg_cmspath.'\/functions$#i', '', GEMELINC);
if($cfg_multi_site == 'Y')
{
...
$cfg_softname = '政元软件网站管理系统';
$cfg_soft_enname = 'gemelcms';
$cfg_soft_devteam = 'Gemel Team';
...
function __autoload($classname)
{
    ...
    $libclassfile = $classname.'.class.php';
        if ( is_file ( GEMELINC.'/'.$libclassfile ) )
        {
            require GEMELINC.'/'.$libclassfile;
        }
        else if( is_file ( GEMELMODEL.'/'.$classfile ) ) 
        {
            require GEMELMODEL.'/'.$classfile;
        }
        else
        {
            if (DEBUG_LEVEL === TRUE)
...
{
    require_once(GEMELINC.'/gemelsqli.class.php');
} else {
    require_once(GEMELINC.'/gemelsql.class.php');
}
...
// 模块MVC框架需要的控制器和模型基类
require_once(GEMELINC.'/control.class.php');
require_once(GEMELINC.'/model.class.php');

//载入小助手配置,并对其进行默认初始化
if(file_exists(GEMELDATA.'/helper.inc.php'))
{
    require_once(GEMELDATA.'/helper.inc.php');
