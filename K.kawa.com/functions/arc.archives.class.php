...
require_once(GEMELINC."/channelunit.class.php");
require_once(GEMELINC."/downmix.inc.php");
...
class Archives
{
    ...
    {
        //读取附加表信息，并把附加表的资料经过编译处理后导入到$this->Fields中，以方便在模板中用 {gemel:field name='fieldname' /} 标记统一调用
        if($this->ChannelUnit->ChannelInfos['addtable']!='')
        {
        ...
        if (isset($this->Fields['litpic']))
        {
            ...
            //模板里直接使用{gemel:field name='image'/}获取缩略图
        ...
        else
        {
            for($i=1;$i<=$this->TotalPage;$i++)
            {
                ...
                if($cfg_remote_site=='Y' && $isremote == 1)
                {
                    ...
                    //分析远程文件路径
                    $remotefile = str_replace(GEMELROOT, '', $TRUEfilename);
    ...
    function GetPageTitlesDM($styleName, $pageNo)
    {
        ...
        {
            $revalue = "<select id='gemelpagetitles' onchange='location.href=this.options[this.selectedIndex].value;'>\r\n";
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
