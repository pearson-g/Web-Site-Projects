...
function ch_img($fvalue,&$arcTag,&$refObj,$fname='')
{
    ...
    $aid = $refObj->ArcID;
    $row = $refObj->dsql->GetOne("SELECT title,typeid FROM `#@__archives` WHERE `id` = '$aid';");
    $title = $row['title'];
    $typeid2= $row['typeid'];
    $row_type = $refObj->dsql->GetOne("SELECT typename FROM `#@__arctype` WHERE `id` = '$typeid2';");
    $typename2=$row_type['typename'];
    $revalue = '';
    $GLOBAL['photoid'] = 0;
