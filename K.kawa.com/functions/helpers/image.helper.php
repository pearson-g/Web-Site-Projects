...
if ( ! function_exists('WaterImg'))
{
    function WaterImg($srcFile, $fromGo='up')
    {
        ...
        include(GEMELDATA.'/mark/inc_photowatermark_config.php');
        require_once(GEMELINC.'/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = GEMELDATA.'/mark/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(GEMELINC.'/data/fonts/elephant.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  GEMELINC.'/data/fonts/elephant.ttf';
            }
            else
            {
                return ;
