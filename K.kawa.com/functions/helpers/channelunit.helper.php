    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        if(in_array($tagname,$alltags))
        {
           if(in_array($tagname, $disable_tags))
            ...
            $filename = GEMELINC.'/taglib/'.$tagname.'.lib.php';
...
function GetBookUrl($bid,$title,$gdir=0)
{
    global $cfg_cmspath;
    $bookurl = $gdir==1 ?
    "{$cfg_cmspath}/book/".GemelID2Dir($bid) : "{$cfg_cmspath}/book/".GemelID2Dir($bid).'/'.GetPinyin($title).'-'.$bid.'.html';
    return $bookurl;
}
...
function GetCacheBlock($cacheid)
{
    ...
    $cachefile = GEMELDATA.'/cache/'.$cacheid.'.inc';
