...
// 弱不存在缓存文件则写入缓存
if(!file_exists(GEMELDATA.'/enums/system.php')) WriteEnumsCache();
...
function WriteEnumsCache($egroup='')
{
    ...
    {
        $cachefile = GEMELDATA.'/enums/'.$egroup.'.php';
...
{
    global ${'em_'.$egroup.'s'};
    include_once(GEMELDATA.'/enums/'.$egroup.'.php');
    $jsCode = "<!--\r\n";
...
{
    $jsfile = GEMELDATA.'/enums/'.$egroup.'.js';
    $fp = fopen($jsfile, 'w');
