...
{
    $cacheFile = GEMELDATA.'/cache/addon-'.session_id().'.inc';
...
class userLogin
{
    ...
    function checkUser($username, $userpwd)
    {
        ...
        $dsql->SetQuery("SELECT admin.*,atype.purviews FROM `gemel_admin_list` admin LEFT JOIN `gemel_admintype` atype ON atype.rank=admin.usertype WHERE admin.userid LIKE '".$this->userName."' LIMIT 0,1");
        $dsql->Execute();
        $row = $dsql->GetObject();
        if(!isset($row->pwd))
        {
    ...
    function keepUser()
    {
        if($this->userID != '' && $this->userType != '')
        {
            ...
            if(empty($adminstyle)) $adminstyle = 'yingxiaoli.com';
            ...
            PutCookie('GemelUserID', $this->userID, 3600 * 24, '/');
            PutCookie('GemelLoginTime', time(), 3600 * 24, '/');
    ...
    {
        //$this->userChannel
        $cacheFile = GEMELDATA.'/cache/admincat_'.$this->userID.'.inc';
        //管理员管理的频道列表
    ...
    function exitUser()
    {
        ...
        DropCookie('GemelAdmindir');
        DropCookie('GemelUserID');
        DropCookie('GemelLoginTime');
        $_SESSION = array();
    }
...
function GetSonIdsUL($id, $channel=0, $addthis=TRUE)
{
    ...
    if( !is_array($cfg_Cs) )
    {
        require_once(GEMELDATA."/cache/inc_catalog_base.inc");
    }
