...
class SplitWord
{
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists(GEMELINC.'/'.$this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    {
		$this->addonDicFile = GEMELINC.'/'.$this->addonDicFile;
		$this->mainDicFile = GEMELINC.'/'.$this->mainDicFile;
		$this->mainDicFileZip = GEMELINC.'/'.$this->mainDicFileZip;
	...
	function InportDict( $targetfile )
     {
     	if(!ini_set('memory_limit', '128M'))
			exit('设置内存错误!');
     	require_once(GEMELINC.'/zip.class.php');
     	$zip = new zip();
     	//echo $targetfile;
     	$unpackagefile = array_keys($zip->Extract($targetfile,GEMELINC.'/data/'));
     	//exit();
     	$this->MakeDict(GEMELINC.'/data/'.$unpackagefile[0]);
     	unlink(GEMELINC.'/data/'.$unpackagefile[0]);
     	return true;
     }
}
